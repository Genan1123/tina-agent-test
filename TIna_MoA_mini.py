#!/usr/bin/env python3
# Tina MoA Minimal Core – v1.1 (Fixed model usage)
# ------------------------------------------------------------
# Workflow: idea generation → aggregation → multi-model scoring
#            → execute or retry
# ------------------------------------------------------------
...
if not API_KEY:
    raise EnvironmentError("Please set TOGETHER_API_KEY")

client = together.AsyncClient(api_key=API_KEY)

# --- prompt templates --------------------------------------------------------
PLANNER_PROMPT = (
    "You are the planning agent. Given a GOAL, give the best solution for the situation."
)
AGGREGATOR_PROMPT = (
    "Merge the following candidate sub-tasks into ONE concise, actionable sub-task."
)
CRITIC_PROMPT = (
    'Rate the sub-task from 0-10. '
    'Return ONLY valid JSON: {"score":N,"reason":"short reason"}'
)
REACT_PROMPT = (
    """You are the expert executor. Given a sub-task, decide what to do and output a valid JSON.

    Only return valid JSON. No explanations, no markdown.

    If it requires bash execution:
{"action": "execute_bash", "bash": "your_bash_command_here"}

    If it is done:
{"action": "task_complete", "result": "short summary"}

    If reasoning only:
{"action": "think", "thought": "your reasoning"}

    You MUST NOT return prose or formatted text.
"""
)
...
async def main() -> None:
    ...
    while not validated:
        ...
        # 3) scoring
        critic_outputs = await asyncio.gather(
            *[llm(m, CRITIC_PROMPT, subtask, temperature=0) for m in CRITIC_MODELS]
        )
